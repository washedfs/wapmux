...
def handle_audio(
    ...
    if src_path.suffix.lower() != ".mkv" or src_path.suffix.lower() != ".m2ts":
    ...
    for i, track_num in enumerate(track_list):
        ...
        if audio_format.should_not_transcode():
            audio_track = create_audio_track(audio_file, name_scheme, d)
            audio_tracks.append(audio_track)
        else:
            if audio_format.display_name.lower() == "pcm" or transcode_all:
                audio_track = create_audio_track(FLAC(preprocess=None).encode_audio(audio_file), name_scheme, d)
                audio_tracks.append(audio_track)
    return audio_tracks
...
def create_audio_track(
    ...
    audio_track = audio_file.to_track(track_name, lang_tag, default=True, forced=False, delay=container_delay)
    return audio_track